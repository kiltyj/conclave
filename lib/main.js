...
const cheerpjWorker = new CheerpJWorker();

const waitForDb = (obj, prop, interval = 100) => {
...
const waitForFileDeletion = async (filename) => {
...
const plantuml = (() => {
    const initialize = async (cheerpjPath = "/app/plantuml-wasm") => {
      ...
      await cheerpjWorker.cheerpjRunMain("com.plantuml.wasm.v1.RunInit", `${cheerpjPath}/plantuml-core.jar`, `${cheerpjPath}/`)        
    }

    const renderPng = async (pumlContent) => {
      const renderingStartedAt = new Date()
      const resultFileSuffix = renderingStartedAt.getTime().toString()
      const tempFilename = `result-${resultFileSuffix}.png`;
      const result = await cheerpjWorker.cjCall("com.plantuml.wasm.v1.Png", "convert", "light", `/files/${tempFilename}`, pumlContent);
      const obj = JSON.parse(result);
      if (obj.status !== 'ok') {
        throw obj.error;
      }
      const blob = await cheerpjWorker.cjFileBlob(tempFilename);
      await waitForFileDeletion(tempFilename);
...
function renderPng(srcText){
  if (srcText == null || srcText.trim() == '') {
    srcText = 'node "PlantUML is ready for input  "';
  }
  const dpiLine = 'scale 4\n';
  const pattern = /\@start.*/;
  const matches = pattern.test(srcText);
  console.log({matches});
  if (matches) {
    console.log(srcText);
    srcText = srcText.replace(/(\@start.*)/, (a, b) => {
      return `${b}\n${dpiLine}`;
    });
    console.log(srcText);
  } else {
    srcText = dpiLine + srcText;
  }
  plantuml.renderPng(srcText).then((blob) => {
    document.getElementById('renderImage').src = window.URL.createObjectURL(blob)
  }).catch((e) => {
    console.log('Did not update PlantUML diagram:', e);
  })
}
...
const debouncedRender = debounce((srcText) => renderPng(srcText));
...
plantuml.initialize(jarPath)
  ...
  .catch((e) => {
    console.error('Could not initialize PlantUML:', e);
  });
